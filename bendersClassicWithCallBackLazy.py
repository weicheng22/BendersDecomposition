...
def subProblem(x):
    ...
    m1.update()
    m1.Params.OutputFlag = 0  # Disable output log
    m1.Params.InfUnbdInfo = 1  # Required to obtain Farkas dual
    m1.Params.DualReductions = 0  # Disable dual reductions

    m1.optimize()

    if m1.status == GRB.OPTIMAL:  # Sub-problem is feasible
        mu = {}
        nu = {}
        for i in range(C):
            mu[i] = constrMu[i].pi  # Get the Dual value of current solution (also known as the shadow price)
        for j in range(F):
            for i in range(C):
                nu[i, j] = constrNu[i, j].pi  # Get the Dual value of current solution (also known as the shadow price)
        return m1.objVal, mu, nu, [y[i, j].x for i in range(C) for j in range(F)], m1.status
    else:  # Sub-problem is infeasible
        mu = {}
        nu = {}
        for i in range(C):
            mu[i] = constrMu[i].FarkasDual  # Get the Farkas infeasibility ray of current solution
        for j in range(F):
            for i in range(C):
                nu[i, j] = constrNu[i, j].FarkasDual  # Get the Farkas infeasibility ray of current solution
...
def setupMasterProblemModel():
    ...
    m.update()
    m.Params.OutputFlag = 0  # Disable output log
...
def callBackFunction(model, where):
    if where == GRB.Callback.MIPSOL:
        ...
        if status == 3:  # Sub-problem is infeasible
            tot = sum(mu.values())
            for j in range(F):
                for i in range(C):
        ...
        if status == 2:  # Sub-problem is feasible
...
def solveUFLBenders(eps, x_initial, maxit, verbose=0):
    ...
    fesCuts_mu = []
    fesCuts_nu = []
    tol = float("inf")  # tolerance
    it = 0  # Iteration counter
    x = x_initial  # Fixed x
    m = setupMasterProblemModel()  # Setup master problem
    while eps < tol and it < maxit:
        ob, mu, nu, y, status = subProblem(x)
        LB = max(LB, ob)
        if status == 2:  # Sub-problem is feasible and get optimal solution
